...
{
    public class OpenApiAuthenticationSchemeOptions : CookieAuthenticationOptions
    {
        public OpenApiAuthenticationSchemeOptions() : base()
        {

        }
    }
    public class OpenAuthenticationHandler : CookieAuthenticationHandler
    {
        public OpenAuthenticationHandler(IOptionsMonitor<CookieAuthenticationOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock) : base(options, logger, encoder, clock)
        {
        ...
        private async Task<AuthenticateResult> ReadCookieTicket()
        {
            ...
            if (this.Context.Request.Cookies.ContainsKey(this.Options.Cookie.Name))
            {
                token = this.Context.Request.Cookies[this.Options.Cookie.Name];
            }
            else if (this.Context.Request.Query.ContainsKey(this.Options.Cookie.Name))
            {
                token = this.Context.Request.Query[this.Options.Cookie.Name];
            }
            else
            {
                token = this.Context.Request.Headers[this.Options.Cookie.Name].FirstOrDefault();
            }
            ...
            var ticket = Options.TicketDataFormat.Unprotect(token);
            if (ticket == null)
            {
    ...
    public class OpenApiAuthenticationHandler :        AuthenticationHandler<OpenApiAuthenticationSchemeOptions>,        IAuthenticationSignInHandler,        IAuthenticationSignOutHandler
    {
        ...
        public OpenApiAuthenticationHandler(IOptionsMonitor<OpenApiAuthenticationSchemeOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock) : base(options, logger, encoder, clock)
        { }
